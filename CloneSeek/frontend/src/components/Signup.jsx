...
function Signup() {
  ...
  const handleSignup = async () => {
    ...
    try {
      ...
      console.log("Full error response:", error.response); // Debugging line
      const msg = error.response?.data?.errors?.[0] || // If array
                  error.response?.data?.errors ||      // If string
                  error.response?.data?.message ||    // Alternative key
                  "Signup Failed";
      setError(msg);
    } finally {
      setLoading(false);
